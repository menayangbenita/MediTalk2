                ...
                <div class="card mb-5 mb-xl-5 box-shadow">
                    ...
                    <div class="card-body p-9 pt-0">
                        <div class="table-responsive">
                            <table id="kt_konsultasi_table" class="table align-middle table-row-dashed fs-6 gy-5">
                                <thead>
                                ...
                                <tbody class="fw-semibold text-gray-600 text-start text-wrap">
                                    ...
                                    @foreach ($riwayats as $riwayat)
                                        <tr>
                                            <td>{{ \Carbon\Carbon::parse($riwayat->waktu_mulai)->translatedFormat('d F Y') }}
                                            </td>
                                            <td>{{ \Carbon\Carbon::parse($riwayat->waktu_mulai)->format('H.i') }} -
                                                {{ \Carbon\Carbon::parse($riwayat->waktu_selesai)->format('H.i') }}</td>
                                            <td>dr. {{ $riwayat->dokter->nama }}</td>
                                            <td>{{ $riwayat->catatan ?: '-' }}</td>
                                            <td class="d-flex">
                                                <a href="{{ route('pasien.chat', $riwayat->id) }}"
                                                    class="btn btn-icon btn-circle btn-sm  btn-light-info p-2 me-2"
                                                    data-bs-toggle="tooltip" data-bs-placement="top" title="Lihat chat">
